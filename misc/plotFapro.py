...
class plotEnviroscan(object):
    def __init__(self, backgroundFilenameEnd, roiFilenameEnd):
        ...
        for i in range(0, 999):
            fileBackground = 'cluster_finalDrop_%03d_%s' % (i, backgroundFilenameEnd)
	    if os.path.isfile(fileBackground) : break

        for j in range(i, 999):
            try:
                fileBackground = 'cluster_finalDrop_%03d_%s' % (j, backgroundFilenameEnd)
		if os.path.isfile(fileBackground) == False : break
                dataBackgroundTmp = self.importFile(fileBackground)
                fileROI = 'cluster_finalDrop_%03d_%s' % (j, roiFilenameEnd)
		if os.path.isfile(fileROI) == False : break
		fsize = os.path.getsize(fileROI)
		if fsize < 6 : continue
                ...
                self.dataBackground[j] = dataBackgroundTmp
                self.dataROI[j] = dataROITmp
            except IOError:
                continue
...
p = plotEnviroscan('1_beforeROI.txt', '2_ROI.txt')
p.show()
